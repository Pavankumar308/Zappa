...
class ZappaCLI(object):
    ...
    def dispatch_command(self, command, environment):
        ...
        try:
            self.load_settings(self.vargs.get('settings_file', 'zappa_settings.json'))
...
╚══════╝╚═╝  ╚═╝╚═╝     ╚═╝     ╚═╝  ╚═╝\n""", fg='green', bold=True))
    ...
    def load_settings(self, settings_file=None, session=None):
        ...
        # custom zappa settings key for aws event bride
    ...
    def create_package(self, output=None):
        ...
        with zipfile.ZipFile(handler_zip, 'a') as lambda_zip:
            ...
            if self.event_bridge_handler:
                settings_s += "EVENT_BRIDGE_HANDLER='{0!s}'\n".format(
                    self.event_bridge_handler)
            else:
